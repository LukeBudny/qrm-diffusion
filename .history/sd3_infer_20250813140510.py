...
class SD3Inferencer:
    ...
    def find_final_scalar_linear(self,m: nn.Module) -> nn.Linear:
    ...
    @torch.no_grad()
    def get_penultimate_features(self,model: nn.Module, call_fn, *args, **kwargs) -> torch.Tensor:
        """
        ...
        last_linear = self.find_final_scalar_linear(model)
    ...
    def imagereward_penult_batch(self,prompts, pil_images):
        feats = []
        for p, im in zip(prompts, pil_images):
            def call():
                # this calls the model in the usual way; the hook above catches the penultimate
                return self.reward_model.score(p, im)  # works whether this returns torch or python float
            f = self.get_penultimate_features(self.reward_model, call)
    ...
    def hps_penult_batch(self,prompts, pil_images, hps_version="v2.1"):
        feats = []
        for p, im in zip(prompts, pil_images):
            ...
            f = self.get_penultimate_features(model, call)
    ...
    def load(
        ...
        if load_tokenizers:
            ...
            self.clip_g = ClipG(model_folder, "cpu")
        ...
        if load_non_tokenizers:
            ...
            if self.eval_model not in ["clip",  "raw_clip","ir","hps","all"]:
    ...
    def get_vision_feature(self, latent, prompts,text_inputs,inference=False):
        ...
        if eval_model in ["ir", "all"]:
            pil_images = [transforms.ToPILImage()(img.cpu()) for img in image]
            ir_feats = self.imagereward_penult_batch(prompts, pil_images).to(image.device, dtype=image.dtype)
            feats.append(ir_feats)

        if eval_model in ["hps", "all"]:
            pil_images = [transforms.ToPILImage()(img.cpu()) for img in image]
            hps_feats = self.hps_penult_batch(prompts, pil_images, hps_version=getattr(self, "hps_version", "v2.1"))
