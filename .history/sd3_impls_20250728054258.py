...
class BaseModel(torch.nn.Module):
    ...
    def apply_model(self, x, sigma,t_raw=None, c_crossattn=None, y=None,vision_feature=None, skip_layers=[], controlnet_cond=None, q_t_training=False):
...
def sample_dpmpp_2m(model, x, sigmas,text_inputs, extra_args=None):
    ...
    for i in tqdm(range(len(sigmas) - 1), leave=False):
        ...
        if callable(vis_fn):
            ...
            extra_args["vision_feature"] = vis_fn(x, prompt,text_inputs)
            extra_args["uncond_vision_feature"] = vis_fn(x, unprompt,text_inputs)
...
def sample_qrm_one_step(model, x, sigmas_dummy,text_inputs, extra_args=None, num_selected_steps=1, lora_only=False):
    ...
    if callable(vis_fn):
        ...
        if isinstance(unprompt, str):
            unprompt = [unprompt]
        all_prompts = prompt + unprompt
        features,vae_decode_time,clip_processor_time,clip_model_time = vis_fn(x.repeat(2, 1, 1, 1), all_prompts,text_inputs)
        extra_args["vision_feature"], extra_args["uncond_vision_feature"] = features.chunk(2)
    ...
    if lora_only:
        with torch.cuda.amp.autocast(enabled=False):
            guided, q_t,q_t_time,sd35_time,t_raw = model(x_t, sigma.view(B) * s_in, t_raw=t_raw, **clean_args)
    else:
        with torch.cuda.amp.autocast(dtype=torch.float16):
            guided, q_t,q_t_time,sd35_time,t_raw = model(x_t, sigma.view(B) * s_in, t_raw=t_raw, **clean_args)

    return [guided], [q_t], [x_t], [sigma]
