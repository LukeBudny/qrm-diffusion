...
class QRMTrainer_batches:
    def __init__(self, inferencer, device="cuda", lr=1e-5, qrm_checkpoint_path=None,time_bool=True,qrm_type="mlp",
        ...
        self.clip_image_preprocess = T.Compose([
    ...
    def train(self, cfg_scale=4.5, cfg_weight=0.0,contrastive_weight=0.0,clip_weight=0.0,ir_weight=0.0,hps_weight=0.0,start_epoch=0,test = False,model_tag='unnamed?',warmup_q_t_steps=100):
        ...
        if ir_weight > 0.0:
            reward_model = reward.load("ImageReward-v1.0").to(self.device).eval()
            ...
            for step, (images, prompts, cond) in enumerate(
                ...
                if self.lora_only:
                    ...
                    with torch.cuda.amp.autocast(enabled=False):  
                        guided_list, q_t_list, x_t_list, sigma_list,t_raw,c,c2,c3 = sample_qrm_one_step( #sample_dpmpp_2m_qrm OR sample_qrm_one_step
                            denoiser, x_t, sigmas_full, text_inputs, extra_args, num_selected_steps=1, lora_only=True
                        )
                else:
                    with torch.cuda.amp.autocast(dtype=torch.float16):
                        guided_list, q_t_list, x_t_list, sigma_list,t_raw,c,c2,c3 = sample_qrm_one_step(
                ...
                for guided, q_t, x_t_i, sigma_i in zip(guided_list, q_t_list, x_t_list, sigma_list):
                    ...
                    if clip_weight != 0.0 or ir_weight != 0.0 or hps_weight != 0.0:
                        with torch.no_grad():
                            rgb_imgs = self.vae.decode(self.latent_fmt.process_out(x_t_i.float()))
                            rgb_imgs = ((rgb_imgs + 1) / 2).clamp(0, 1)
                            rgb_imgs = F.interpolate(rgb_imgs, size=(224, 224), mode="bicubic", align_corners=False)
                            rgb_imgs = self.clip_image_preprocess(rgb_imgs)

                    if clip_weight != 0.0:
                            # Resize + normalize for CLIP
                            clip_inputs = self.inferencer.clip_tokenizer(
                                prompts, return_tensors="pt", padding=True, truncation=True
                            ).to(self.device)
                            text_embeds = self.inferencer.clip_model.get_text_features(**clip_inputs)
                            image_embeds = self.inferencer.clip_model.get_image_features(pixel_values=rgb_imgs.to(self.device))
                            sim = F.cosine_similarity(image_embeds, text_embeds, dim=-1)
                            reg_loss = (1 - sim.mean()) * clip_weight

                    if ir_weight != 0.0:
                        # ImageReward expects PIL images; convert from tensors
                        pil_images = [T.ToPILImage()(img.cpu()) for img in rgb_imgs]
                        scores = []
                        with torch.no_grad():
                            for p, pil_img in zip(prompts, pil_images):
                                s = reward_model.score(p, pil_img)  # already float in most versions
                                scores.append(float(s))
                        # Maximize reward → minimize negative
                        reward_tensor = torch.tensor(scores, device=self.device, dtype=rgb_imgs.dtype)
                        reg_loss = (-reward_tensor.mean()) * ir_weight
                    ...
                    if hps_weight != 0.0:
                        ...
                        with torch.no_grad():
                            for p, pil_img in zip(prompts, pil_images):
                                s = hpsv2.score(pil_img, p, hps_version="v2.1")[0]
                                scores.append(float(s))
                        hps_tensor = torch.tensor(scores, device=self.device, dtype=rgb_imgs.dtype)
                        # Assuming hpsv2.score returns ~0..1; if ~0..100 adjust accordingly
                        reg_loss = (1.0 - hps_tensor.mean()) * hps_weight
                ...
                print(f"[E{epoch} S{step}] loss={(loss.item() * self.accum_steps):.4f} | step_time={time.time() - step_start:.2f}s | clip loss={reg_loss}")
