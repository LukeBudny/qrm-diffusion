...
class QRMTrainer_batches:
    ...
    def train(self, cfg_scale=4.5, cfg_weight=0.5,contrastive_weight=0.5,clip_weight=0.5,start_epoch=0,test = False,model_tag='unnamed?',warmup_q_t_steps=100):
        ...
        for epoch in range(start_epoch+1, start_epoch+self.num_epochs+1):
            ...
            for step, (images, prompts, cond) in enumerate(
                ...
                print(f"[E{epoch} S{step}] loss={(loss.item() * self.accum_steps):.4f} | step_time={time.time() - step_start:.2f}s | weight_t={weight_t}")

            c1_stats_list.append({"q_t+t+y min:", c.min().item(),"max: ", c.max().item(),"mean: ", c.mean().item(),"Nan?: ", torch.isnan(c).any().item(),"norm: ",c.norm().item()})
            c2_stats_list.append({"t+y min:", c2.min().item(),"max: ", c2.max().item(),"mean: ", c2.mean().item(),"Nan?: ", torch.isnan(c2).any().item(),"norm: ",c2.norm().item()})
            c3_stats_list.append({"q_t min:", c3.min().item(),"max: ", c3.max().item(),"mean: ", c3.mean().item(),"Nan?: ", torch.isnan(c3).any().item(),"norm: ",c3.norm().item()})
            ...
            self.save_training_log(log_file, epoch_list, loss_list, time_list, q_t_stats_list)

        total_time = time.time() - overall_start
        print(f"🚀 Training complete in {total_time / 60:.2f} minutes")

    def save_training_log(self, log_file, epoch_list, loss_list, time_list, q_t_list):
        history = {
            "epoch": epoch_list,
            "loss": loss_list,
            "time": time_list,
