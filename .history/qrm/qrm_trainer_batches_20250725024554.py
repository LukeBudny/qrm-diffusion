...
class QRMTrainer_batches:
    ...
    def train(self, cfg_scale=4.5, cfg_weight=0.5,contrastive_weight=0.5,clip_weight=0.5,start_epoch=0,test = False,model_tag='unnamed?'):
        print("batch_size ", self.batch_size)
        epoch_list, loss_list, time_list, q_t_stats_list = [], [], [], []
        ...
        for epoch in range(start_epoch+1, start_epoch+self.num_epochs+1):
            ...
            for step, (images, prompts, cond) in enumerate(
                ...
                for guided, q_t, x_t_i, sigma_i in zip(guided_list, q_t_list, x_t_list, sigma_list):
                    ...
                    # # Optional: add time-dependent loss weighting
                    # t_fn = lambda sigma: -sigma.log()
                    # t = t_fn(sigma_i)  # shape: [B]
                    # weight_t = 1.0 / (1.0 - t).view(-1, 1, 1, 1)

                    # Weighted MSE loss between predicted and target ε
                    # main_loss = 0.5 * weight_t * (predicted_epsilon - target_epsilon).pow(2).mean()
                    # total_loss += main_loss
            ...
            q_t_stats_list.append({"mean": float(np.mean(q_t_means)), "std": float(np.mean(q_t_stds))})
