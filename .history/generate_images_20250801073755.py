...
def compute_batched_vision_features_for_step(inferencer, latents_batch, prompts_batch, text_inputs=None):
    ...
    with torch.no_grad():
        features_all = inferencer.get_vision_feature(latents_all, prompts_all, text_inputs,inference=True)
    ...
    for model_name, qrm_ckpt in MODELS.items():
        ...
        for metadata_batch in chunked(metadata_list, BATCH_SIZE):
            ...
            if qrm_ckpt is not None:
                ...
                # Current allocated VRAM (used by tensors)
                current_vram = torch.cuda.memory_allocated() / (1024 ** 2)  # in MB

                # Peak allocated VRAM since the last reset
                max_vram = torch.cuda.max_memory_allocated() / (1024 ** 2)  # in MB

                # Total VRAM on the GPU
                total_vram = torch.cuda.get_device_properties(0).total_memory / (1024 ** 2)  # in MB

                print(f"Initial Current VRAM: {current_vram:.2f} MB")
                print(f"Initial Max VRAM: {max_vram:.2f} MB")
                print(f"Initial Total VRAM: {total_vram:.2f} MB")
                ...
                print("latents_batch shape: ",latents_batch.shape)
                print("finished initial latent batch generation: ",time.time() - start)
                ...
                print(f"After Latent Batch Initialization Current VRAM: {current_vram:.2f} MB")
                print(f"After Latent Batch Initialization Max VRAM: {max_vram:.2f} MB")
                print(f"After Latent Batch Initialization Total VRAM: {total_vram:.2f} MB")
                ...
                for step_idx in range(len(sigmas) - 1):
                    ...
                    print("finished batch vision feature generation: ",time.time() - start)
                    ...
                    print(f"After Batch Vision Feature Initialization Current VRAM: {current_vram:.2f} MB")
                    print(f"After Batch Vision Feature Initialization Max VRAM: {max_vram:.2f} MB")
                    print(f"After Batch Vision Feature Initialization Total VRAM: {total_vram:.2f} MB")
                    ...
                    for img_idx in range(len(prompts_batch)):
                        ...
                        print(next(denoiser.parameters()).device)
                        print(latents_batch[img_idx:img_idx+1].device)
                        print(sigmas.device)
                        print(vf_batch[img_idx:img_idx+1].device)
                        print(uvf_batch[img_idx:img_idx+1].device)
                ...
                print("total_batch generation_time:",time.time() - start_batch)
