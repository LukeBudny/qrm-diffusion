...
def _ensure_batched(x):
    return x.unsqueeze(0) if x.ndim == 2 else x  # [L,D]->[1,L,D] or [D]->[1,D]

def _norm_cond_dict(d):  # d has keys c_crossattn, y
    c = d["c_crossattn"]
    y = d["y"]
    return {
        "c_crossattn": _ensure_batched(c),
        "y": _ensure_batched(y),
    }
...
def load_cached_map(cache_dir):
    ...
    if os.path.isfile(packed):
        ...
        if isinstance(obj, dict):
            for k, v in obj.items():
                mapping[k] = _norm_cond_dict(v)
        elif isinstance(obj, (list, tuple)):
            for d in obj:
                mapping[d["prompt"]] = _norm_cond_dict(
                    {"c_crossattn": d["c_crossattn"], "y": d["y"]}
                )
        else:
    ...
    uncond = None
    up = os.path.join(cache_dir, "uncond.pt")
    if os.path.isfile(up):
        uncond = _norm_cond_dict(torch.load(up, map_location="cpu"))
