...
def main():
    ...
    metadata_map = {m["prompt"]: m for m in metadata_list}
    ...
    prompt_order_map = {p: i for i, p in enumerate(prompt_order)}
    sorted_batch = sorted(loader, key=lambda x: prompt_order_map.get(x[0], float('inf')))

    for idx, (prompt, cond, uncond) in enumerate(CachedCondsIterable(CACHE_DIR)):
        print(f"{idx:04d}: {prompt}")
        ...
        for i, (prompt, cond_cpu, uncond_cpu) in enumerate(loader):
            ...
            if not os.path.exists(meta_path):
                with open(meta_path, "w") as f:
                    full_meta = metadata_map[prompt[0] if isinstance(prompt, tuple) else prompt]
                    f.write(json.dumps(full_meta) + "\n")
