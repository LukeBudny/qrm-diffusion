...
from transformers import CLIPTokenizer, CLIPModel,CLIPProcessor ,BlipProcessor, BlipForImageTextRetrieval,AutoProcessor, AutoModel
...
class SD3Inferencer:
    ...
    def load(
        ...
        if load_non_tokenizers:
            ...
            self.one_step_sample = getattr(sd3_impls, "sample_{sampler}")
    ...
    @torch.no_grad()
    def get_vision_feature(self, latent, prompts,text_inputs):
        ...
        if eval_model in ["blip", "all"]:
            self.blip_model = self.blip_model.to(image.device)
            pil_images = [transforms.ToPILImage()(img.cpu()) for img in image]
            inputs = self.blip_processor(images=pil_images, text=prompts, return_tensors="pt").to(image.device)
            itm_logits = self.blip_model(**inputs)[0]
            probs = torch.softmax(itm_logits, dim=-1)[:, 1:]  # [B, 1]
            feats.append(probs)

        if eval_model in ["dinov2", "all"]:
            self.dino_model = self.dino_model.to(image.device)
            inputs = self.dino_processor(images=image, return_tensors="pt").to(image.device)
            dino_feat = self.dino_model(**inputs).last_hidden_state.mean(dim=1)
            feats.append(dino_feat.to(dtype=image.dtype, device=image.device))
    ...
    @torch.no_grad()
    def get_vision_feature_new(self, latent,prompts, text_inputs):
        clip_time,vae_decode_time,preprocess_clip_time =0,0,0
        eval_model = self.eval_model
        # start = time.time()
        with torch.cuda.amp.autocast(dtype=torch.float16):
            latent = latent.to(dtype=self.vae.model.decoder.conv_in.weight.dtype)
        ...
        # vae_decode_time = time.time() - start
        ...
        if eval_model in ["clip", "raw_clip", "all"]:
            ...
            # start = time.time()
            image = batched_clip_preprocess(image)
            # preprocess_clip_time = time.time() - start
            # start = time.time()
            out = self.clip_model(pixel_values=image, input_ids=text_inputs["input_ids"], attention_mask=text_inputs["attention_mask"])
            ...
            if eval_model == "clip":
                img_n = img_e / img_e.norm(dim=-1, keepdim=True)
                txt_n = txt_e / txt_e.norm(dim=-1, keepdim=True)
                # print(">>> Prompt batch size:", len(prompts), "Image batch size:", image.shape[0])
                return img_n * txt_n,vae_decode_time,preprocess_clip_time,clip_time
            elif eval_model == "raw_clip":
                return torch.cat([img_e, txt_e], dim=-1),vae_decode_time,preprocess_clip_time,clip_time
            elif eval_model == "all":
                img_n = img_e / img_e.norm(dim=-1, keepdim=True)
                txt_n = txt_e / txt_e.norm(dim=-1, keepdim=True)
                feats.append(img_n * txt_n)
        ...
        if feats:
            ...
            return torch.cat(feats, dim=-1),vae_decode_time,clip_time
        else:
            raise ValueError(f"Unknown or unsupported vision feature model: '{eval_model}'")
    ...
    def do_sampling(
        ...
        text_inputs = ""
        # clip_tokenizer_tokenizer_time = time.time() - start


        # start = time.time()
        latent = sample_fn(denoiser, noise_scaled, sigmas, text_inputs, extra_args=extra_args)
        latent = SD3LatentFormat().process_out(latent)
        # print(f"[Timer] qrm_feature_time forward pass: {qrm_feature_time_total:.2f}s")
        # print(f"[Timer] vae_decode_time forward pass: {vae_decode_time_total:.2f}s")
        # print(f"[Timer] clip_preprocessor_tokenizer_time forward pass: {clip_processor_time_total:.2f}s")
        # print(f"[Timer] clip_model_time forward pass: {clip_model_time_total:.2f}s")
        # print(f"[Timer] q_t_time forward pass: {q_t_time_total:.2f}s")
        # print(f"[Timer] sd35_time forward pass: {sd35_time_total:.2f}s")
        # print(f"[Timer] total forward pass: {time.time() - start:.2f}s")
    ...
    def gen_image(
        ...
        for i, prompt in pbar:
            ...
            name = save_names[i] if save_names else f"{i:06d}.png"
