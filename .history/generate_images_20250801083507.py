        ...
        for metadata_batch in chunked(metadata_list, BATCH_SIZE):
            ...
            if qrm_ckpt is not None:
                ...
                latents_batch = torch.cat([
                    inferencer.get_empty_latent(1, 512, 512, SEED + i,"cpu").cuda().half()
                ...
                print("finished initial latent batch generation: ",time.time() - start ,"seconds")

                # Sigmas for sampling
                sigmas = inferencer.get_sigmas(inferencer.sd3.model.model_sampling, 40).cuda()
                sigmas = sigmas[int(40 * (1 - .8)) :]

                for idx,latent in enumerate(latents_batch):
                    noise = inferencer.get_noise(42, latent).cuda()
                    latents_batch[idx] = inferencer.sd3.model.model_sampling.noise_scaling(sigmas[0], noise, latent, inferencer.max_denoise(sigmas))
                ...
                for step_idx in range(len(sigmas) - 1):
                    ...
                    start = time.time()
                    vf_batch, uvf_batch = compute_batched_vision_features_for_step(inferencer, latents_batch, prompts_batch)
                    print("finished batch vision feature generation: ",time.time() - start,"seconds")
                    
                    # 2. Loop images sequentially for diffusion step
                    for img_idx in range(len(prompts_batch)):
                        extra_args = {
                            "cond": inferencer.fix_cond(inferencer.get_cond(prompts_batch[img_idx])).cuda(),
                            "uncond": inferencer.fix_cond(inferencer.get_cond("")).cuda(),
                            "cond_scale": 4.5,
                        ...
                        def dynamic_vision_fn(x_t, prompt,text_inputs):
                            return inferencer.get_vision_feature(x_t, prompt,text_inputs)
                        extra_args["dynamic_vision_fn"] = dynamic_vision_fn
                        extra_args["prompt"] = prompts_batch[0]
                        ...
                        _ = inferencer.sample_dpmpp_2m(denoiser,latents_batch[img_idx:img_idx+1],sigmas,"",extra_args)
                        print("THIS WORKED #####################################################################################")
                ...
                print("total_batch generation_time:",time.time() - start_batch,"seconds")
