...
def main():
    ...
    for model_name, qrm_ckpt in MODELS.items():
        ...
        for i, (prompt, cond_cpu, uncond_cpu) in enumerate(loader):
            ...
            for k in ["c_crossattn", "y"]:
                if uncond_cpu[k].dim() == 4 and uncond_cpu[k].shape[1] == 1:
                    uncond_cpu[k] = uncond_cpu[k].squeeze(1)
            ...
            def to_cuda_fp16(d):
                ...
                # If cached without batch dim, unsqueeze here:
            ...
            for variant in range(NUM_VARIANTS):
                ...
                inferencer.gen_image(
                    prompts=[prompt],                  # list[str], length 1
