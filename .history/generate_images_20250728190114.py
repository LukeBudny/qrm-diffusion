...
def main():
    ...
    for model_name, qrm_ckpt in MODELS.items():
        ...
        sample_id = 0
        count = 0
        for metadata in tqdm(metadata_list, desc=f"Prompts for {model_name}"):
            ...
            for variant in range(NUM_VARIANTS):
                ...
                if (count == 0 and not os.path.exists(save_path)) or checkpoint is None:
                    ...
                    if qrm_ckpt is not None:
                        if "model" in checkpoint:  # New-style checkpoint
                        ...
                        # inferencer.sd3.model.qrm = QRMRegistry["mlp"](time_bool=True)
                        # inferencer.sd3.model.qrm.load_state_dict(checkpoint)
                        print("⚠️ Legacy checkpoint format detected — assuming QRMMLP with time_bool=True")
                ...
                if not os.path.exists(save_path):
                    ...
                    print(f"⚠️ Missing image at {save_path} — generating it...")
